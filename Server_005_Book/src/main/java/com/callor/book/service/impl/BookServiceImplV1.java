	...
	public BookDTO findById(String bk_isbn) {
		...
		try {
			...
			if(result.next()) {
				...
				bookDTO.setCp_title(result.getString("출판사명"));
				bookDTO.setCp_ceo(result.getString("출판사대표"));
				bookDTO.setAu_name(result.getString("저자명"));
				bookDTO.setAu_tel(result.getString("저자연락처"));
				bookDTO.setBk_date(result.getString("출판일"));
			}
