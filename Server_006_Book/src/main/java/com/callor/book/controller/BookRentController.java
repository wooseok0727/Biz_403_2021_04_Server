	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (subPath.equals("/seq")) {
			...
			if (strSeq == null || strSeq.equals("")) {
				 ...
				 *  요청(Req) 응답(Resp)를 총괄하는 정보가 담긴 객체
				 *  
				 *  Web App Service를 구현하기 위하여 
				 *  Req, Resp 를 처리하는 여러가지 기능을 구현해야 하는데
				 *  그러한 기능을 미리 구현해 놓았기 때문에
				 *  ServletContext를 getter 하는 것만으로 충분하다
				 *  
				 *  DB 등으로부터 조회된 데이터를 Web 에게
				 *  응답하고자 할때 쉬운 방법으로 전달할 수 있도록
				 *  구현하는 기능이 미리 구현되어 있다
				 */
				...
				// Rendering을 하라 = BOOK에 담긴 data를 html 또는 jsp로 변환하라
				// wepapp/WEB-INF/views/book.jsp 파일을 읽어서
				// Java코드로 변환하고, 실행할 준비를 하라
...
//				System.out.println("=".repeat(50));
			...
			// 현재 시스템 날짜 가져오기 (System.currentTimeMillis())
