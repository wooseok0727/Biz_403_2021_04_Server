...
public class BookRentServiceImplV1 implements BookRentService {
	 ...
	 * 		"직접" exception을 처리하기
	 * 2. 떠넘기기(던지기)
	 * 		method에 throws 를 추가하여
	 * 		호출한 method에게 exception 처리를 맡긴다
	 * 		현재 method에서 직접 exception을 처리하는 
	 * 		코드가 없어서 다소 간소화 된다
	 * 
	...
	protected List<BookRentDTO> select(PreparedStatement pStr) throws SQLException {
		...
		while(rStr.next()) {
			...
			brDTO.setBu_bname(rStr.getString("회원명"));
			brDTO.setBu_tel(rStr.getString("회원연락처"));
			
			brDTO.setBr_isbn(rStr.getString("ISBN"));
			brDTO.setBk_title(rStr.getString("도서명"));
	...
	public BookRentDTO findById(Long seq) {
		...
		PreparedStatement pStr = null;
		try {
			// 아래의 2개 코드에 의해서 WHERE 주문번호 = 값 형식의
			// SQL이 만들어진다
			pStr = dbConn.prepareStatement(sql);
			pStr.setLong(1, seq);
...
//			rSet.close();
	...
	public int insert(BookRentVO bookRentVO) {
		...
		try {
			...
			// inser, update, delete 와 관련된 SQL은
