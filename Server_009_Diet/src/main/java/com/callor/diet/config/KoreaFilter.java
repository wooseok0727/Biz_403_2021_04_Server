 ...
 * login 여부에 따라 Contorller로 전달할지 안할지를 
 * 판단하여 미리 한번에 처리 할수 있다.
 */
...
/*
 * Filter에서 resp.setContentType()을 지정할 때
 * text/html 로 지정을 하면 
 * jsp, js, 기타 여러가지 static 파일들이
 * 모두 html ContentType으로 지정되어
 * WebBrowser로 전송되어 제 기능을 수행하지 못하는 현상이 발생한다
 * 
