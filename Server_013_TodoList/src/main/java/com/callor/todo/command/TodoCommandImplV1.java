...
public class TodoCommandImplV1 implements TodoCommand {
	...
	// 생성을 할 때 관리이름으로 "TODO"를 부착하라
	// "TODO"라는 이름으로 Logger를 싱글톤으로 제공하라
	// Factory 패턴
	...
	public void execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(path.equals("/insert")) {
			...
			// 1 이상이면 정상 insert이고
			// 그렇지 않으면 추가가 잘못된것
			Integer result = toService.insert(tdVO);
		...
		// Map type의 VO에 현재 날짜, 시간, 할일 정보를 저장하기
		// VO에 칼럼(key)을 추가하면서 동시에 값 저장하기
...
//		} else {
//			req.setAttribute("COMPLETE", "INSERT 성공!!");
//		}
		// "/todo/" 
